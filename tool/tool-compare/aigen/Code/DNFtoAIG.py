"""
This file implements the DNF approach. First it generates for each 
latch a random truth table, i.e. a bit vector that determines the 
satisfying assignments. Then it converts the truth table to a set of AND gates,
that are in turn witten in an AIGER file.
@author: msakr
"""
import time
import sys
...
"""
this function takes as input a bit vector bitVec and generates the 
corresponding AND gates. If an element in the vector contains 1/0, then
this indicates that the variable with index equal to the element appears 
in a positive/negative manner in the minterm.
"""
...
"""
This function converts a truth table represented in a bitvector to a Boolean 
with AND and NOT gates. For instance if the 4th element of the truth table
is equal to 1, and the truth table contains three elements that means the 
minterm 100 is a satisfying assignment.
"""
    ...
    for i in range(0, len(bitVec)):
        if bitVec[i] == 1:
            ...
            if not moreThanOne and counter > 1:
                moreThanOne = True
            #This is an optimisation in order not to construct one very large AND statement
            if counter == 100000:
                counter = 0
...
"""
This function converts the andGatesDNF table to AIGER format.
"""
...
it takes as input the filename to be generated, the aiger file characteristics,
 and the seeds. Then it generates the corresponding AIGER file
"""
